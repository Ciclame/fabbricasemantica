...
public class TaskMyAnnotationServlet extends BaseServlet {
	...
	/**
	 * Il metodo viene invocato dopo la chiamata alla servlet all'indirizzo 'myAnnotation.jsp' 
	 * e:
	 * <ul>
	...
	protected void doSomething(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(request.getParameter("next") != null && request.getParameter("sinonimi") != "" &&
			request.getParameter("contrari") != "")
		{	
			List<Map<String, Object>> result = Database.executeQuery("INSERT INTO usertask(username, task, risultato) VALUES('"
				+ request.getSession().getAttribute("username") + "', 'myannotation', 'Sinonimi: " 
				+ request.getParameter("sinonimi") + " Contrari: " + request.getParameter("contrari") + " ') returning username");
		...
		{	
				...
				out.println("alert('Fornire almeno un sinonimo e contrario per la parola assegnata prima di premere il pulsante next');");
				out.println("location='MyAnnotation.html';");
				out.println("</script>");
		}
	}
