...
public class TaskDefinitionAnnotationServlet extends BaseServlet 
{
	...
	/**
	 * Il metodo viene invocato dopo la chiamata alla servlet all'indirizzo 'definitionannotation.jsp' 
	 * e:
	 * <ul>
	...
	protected void doSomething(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		if(request.getParameter("next") != null && request.getParameter("definizione") != "")
		{	
			List<Map<String, Object>> result = Database.executeQuery("INSERT INTO usertask(username, task, risultato) VALUES('"
					+ request.getSession().getAttribute("username") + "', 'definitionannotation', " 
					+ "'Definizione fornita: "+ request.getParameter("definizione") + "') returning username");
		...
		else 
		{
			...
			out.println("alert('Dare una definizione appropriata per la parola prima di premere il pulsante next');");
			out.println("location='DefinitionAnnotation.html';");
			out.println("</script>");
		}
	}
}
